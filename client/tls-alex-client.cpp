...
static char lidarBuffer[128];
static int lidarDataLen = 0;
...
void *readerThread(void *conn)
{
	...
	while(networkActive)
	{
		...
		if (len > 0 && buffer[0] == NET_LIDAR_PACKET) {
			...
			lidarDataLen = len;
            memcpy(lidarBuffer, &buffer[0], lidarDataLen);
		}
		/* END TODO */
...
void *lidarwriterThread(void *conn)
{
    while(networkActive)
    {
        if (lidarDataLen > 0)
        {
            // Prepare the packet
            char packet[128];
            memcpy(&packet[0], lidarBuffer, lidarDataLen);

            // Send the LiDAR data
            sendData(conn, packet, lidarDataLen);

        }
        else
        {
            // Handle error or no data condition
            printf("No LiDAR data available.\n");
        }
    }

    printf("Exiting LiDAR writer thread\n");
...
#define SERVER_NAME "192.168.43.183"
...
void connectToServer(const char *serverName, int portNum)
{
 ...
 createClient("127.0.0.1", 5001, 1, CA_CERT_FNAME, SERVER_NAME_ON_CERT, 1, CLIENT_CERT_FNAME, CLIENT_KEY_FNAME, readerThread, lidarwriterThread);
    /* END TODO */
}
